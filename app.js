...
function viewPlan(root) {
  ...
  if (!plan.scored) {
    if (!plan.loading) Promise.resolve().then(() => { if (plan.loc && !plan.scored && !plan.loading && !plan.error) loadPlan(plan.loc); });
    wrap.append(planLoading()); root.append(wrap); return;
  }
...
function viewExplore(root) {
  ...
  import('./worldview.js?v=49').then(m => {
